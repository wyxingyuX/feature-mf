...
def compute_loss(views_X, views_Laplace, views_lamada, views_gamma, alpha1, alpha2, beta, views_U, views_V, V_centroid):
    ...
    for i in range(len(views_X)):
        ...
        loss += 0.5 * np.linalg.norm(X - np.dot(U, V)) \
                + views_lamada[i] * np.dot(np.dot(V_centroid, Laplace), V_centroid.T).trace() \
...
def sim_nmf(views_X, views_lamada, views_gamma, alpha1=0.01, alpha2=0.01, beta=0.01, k=20, max_itr=20, min_error=0.5,
    ...
    while itr < max_itr and (Lt0 - Lt1) > min_error:
        ...
        for i in range(len(views_X)):
            Laplace = views_Laplace[i]
            gradV_centroid += views_lamada[i] * np.dot(V_centroid,(Laplace + Laplace.T)) + views_gamma[i] * (V_centroid - views_V[i])
        ...
        for i in range(len(views_X)):
            ...
            # print len(views_X),i
            gradV_i = alpha2 * V + (np.dot(np.dot(U.T, U), V) - np.dot(U.T, X))+views_gamma[i] * (V - V_centroid)
        ...
        subtGradLoss = compute_loss(views_X, views_Laplace, views_lamada, views_gamma, alpha1, alpha2, beta,
                                    viewU_subGrad, \
